...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 h-fit">
        ...
        <p class="text-xs text-gray-400 mb-5">Créez une remise de 1 à 100 %, utilisable une seule fois par défaut.</p>
        <form method="POST" action="{{ route('admin.promo-codes.store') }}" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3"><div><label class="block text-xs font-bold text-gray-600 mb-1">Remise</label><div class="relative"><input type="number" name="discount_percent" min="1" max="100" value="{{ old('discount_percent', 100) }}" required class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm"><span class="absolute right-3 top-2.5 text-gray-400">%</span></div></div><div><label class="block text-xs font-bold text-gray-600 mb-1">Utilisations</label><input type="number" name="max_uses" min="1" value="{{ old('max_uses', 1) }}" required class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm"></div></div>
